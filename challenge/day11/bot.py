"""Telegram-бот «На Глазок» — КБЖУ по ленивому описанию (Day 11)."""
...
def load_token() -> str:
    ...
    raise RuntimeError(
        "Нет TELEGRAM_BOT_TOKEN. Задай env или challenge/day11/telegram-local.txt"
    )
...
async def cmd_start(message: Message) -> None:
    # /start не стирает долгосрочную память
...
async def cmd_help(message: Message) -> None:
    await message.answer(
        ...
        "/mode baseline|hardened — промпт (day11)"
    )
...
async def safe_edit(status: Message, text: str, *, html: bool = False) -> None:
    """Edit status message; never let Telegram HTML parsing hide the real error."""
    try:
        if html:
...
async def on_food(message: Message, bot: Bot) -> None:
    ...
    try:
        ...
        # plain text — в ошибке часто есть <urlopen ...>, HTML ломает Telegram
        await safe_edit(
            status,
            "Не смог посчитать (сеть/OpenRouter). Попробуй ещё раз через минуту.\n"
...
async def main() -> None:
    token = load_token()
    # без default HTML — иначе ошибки с <> ломают send/edit
    bot = Bot(token=token)
    dp = Dispatcher()
    ...
    log.info("Bot @%s started (aiogram polling)", me.username)
