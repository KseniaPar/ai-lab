"""Day 13 — LLM Gateway (protective proxy) for «На Глазок»."""
...
DAY13_DIR = Path(__file__).resolve().parent
AUDIT_LOG_PATH = DAY13_DIR / "audit.jsonl"
    ...
    if local.is_file():
        ...
        if m:
            ...
            if key and "PASTE" not in key and key != "sk-not-set":
                return key
    raise RuntimeError(
        "API key not found. Set OPENROUTER_API_KEY / REAL_OPENAI_API_KEY "
        "or application-local.yml app.openrouter.api-key"
    )
...
async def proxy_llm_request(request: Request):
    ...
    headers = {
        ...
        "X-Title": "ai-lab-challenge-day13",
        "User-Agent": "ai-lab-day13-gateway/1.0",
    }
